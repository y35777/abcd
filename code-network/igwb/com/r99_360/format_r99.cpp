...
{
    return new CFormat3G(szRootDir, szFmtName);
}
...
//
/***************************************************************/
CFormat3G::CFormat3G(const STRING& sRootDir, const STRING& sFmtName)
...
//
/***************************************************************/
int CFormat3G::InitFormatInfo(void)
{
    //确保本格式域个数大于零
    if(m_uFieldNum == 0)
    {
...
//
/***************************************************************/
int CFormat3G::IsTheFormat(const void* pABill,  const UINT4 uLen)
{
    //不符合本格式
    ...
    if(ACE_OS::strcasecmp(m_sFmtName.c_str(), S_CDR_3GFORMATNAME) == 0)
    {
        ...
        if(pFrontCDR->choice == CDR3G_sgsnPDPRecord_chosen)
        {
            return 0;
        }
        else
    ...
    {
        CDR3G_CallEventRecord* pFrontCDR = (CDR3G_CallEventRecord*)pABill;
        if(pFrontCDR->choice == CDR3G_ggsnPDPRecord_chosen)
        {
            return 0;
        }
        else
    ...
    {
        CDR3G_CallEventRecord* pFrontCDR = (CDR3G_CallEventRecord*)pABill;
        if(pFrontCDR->choice == CDR3G_sgsnMMRecord_chosen)
        {
            return 0;
        }
        else
    ...
    {
        CDR3G_CallEventRecord* pFrontCDR = (CDR3G_CallEventRecord*)pABill;
        if(pFrontCDR->choice == CDR3G_sgsnSMORecord_chosen)
        {
            return 0;
        }
        else
    ...
    {
        CDR3G_CallEventRecord* pFrontCDR = (CDR3G_CallEventRecord*)pABill;
        if(pFrontCDR->choice == CDR3G_sgsnSMTRecord_chosen)
        {
            return 0;
        }
        else
    ...
    {
        _3GCDR_CallEventRecord* pBackCDR = (_3GCDR_CallEventRecord*)pABill;
        if(pBackCDR->choice == _3GCDR_sgsnPDPRecord_chosen)
        {
            return 0;
        }
        else
    ...
    {
        _3GCDR_CallEventRecord* pBackCDR = (_3GCDR_CallEventRecord*)pABill;
        if(pBackCDR->choice == _3GCDR_ggsnPDPRecord_chosen)
        {
            return 0;
        }
        else
    ...
    /*
    //最终话单类型的M-CDR
    if(ACE_OS::strcasecmp(m_sFmtName.c_str(), L_M_CDR_3GFORMATNAME) == 0)
    {
        _3GCDR_CallEventRecord* pBackCDR = (_3GCDR_CallEventRecord*)pABill;
        if(pBackCDR->choice == _3GCDR_sgsnMMRecord_chosen)
        {
            return 0;
        }		
        else
...
//
/***************************************************************/
int CFormat3G::ConvertABill(const void* pASrcBill,
                            ...
                            UINT4& uDestBillLen)
{
    ...
    CDR3G_CallEventRecord *pTemp = (CDR3G_CallEventRecord*)pASrcBill;

    int nLen = 0;
    try
    {
        //把当前话单转换成浏览格式的文本
        nLen = GetAllFieldTextValueOf3GCDR((char*)(&pTemp->u),
                                           m_n3GFormatType,
                                           pBuffer,
                                           _BROWSEBUFFERLENGTH);
    }
...
//
/***************************************************************/
int CFormat3G::GetFieldValueByName(const void* pABill,
                                   const int bThisFmt,
...
//
/***************************************************************/
int CFormat3G::GetFieldContentByIndex(const void* pABill,
                                      const int bThisFmt,
                                      UINT2 nFieldIndex,  
                                      void* pFieldValue,
                                      UINT4& uLen,
...
//
/***************************************************************/
int CFormat3G::IsTheQueryCondition(const void* pASrcBill,
...
//
/***************************************************************/
int CFormat3G::GetQueryCondFieldsValue(const void* pABill)
{
    //获取全局缓冲区
    CGlobalBuffer::acquire();
    char *pBuffer = CGlobalBuffer::GetBillBuffer();
    CDR3G_CallEventRecord *pTemp = (CDR3G_CallEventRecord*)pABill;

    int nLen = 0;
    try
    {
        //获取作为查询条件域的值
        nLen = GetQueryFieldTextValueOf3GCDR((char*)(&pTemp->u),
                                             m_n3GFormatType,
                                             pBuffer,
                                             _BROWSEBUFFERLENGTH,
...
//
/***************************************************************/
int CFormat3G::GetFieldValueByNameEx(const void* pABill,
                                     UINT4& uLen,
                                     const STRING& sFieldName,
                                     void*& pFieldValue)
{
    if(sFieldName == "recordType")
	{
		if (m_bIsMustMerge)
		{
			uLen = sizeof(_3GCDR_CallEventRecordType);
		}
		else
		{
			uLen = sizeof(CDR3G_CallEventRecordType);
		}
...
/***************************************************************/
/***************************************************************/
int CFormat3G::GetFieldContentByNameEx(const void* pABill,
                                       UINT4& uLen,  
                                       const STRING& sFieldName,
                                       void* pFieldValue)
{
    ...
    if(sFieldName == "recordType")
	{
		...
		{
			CDR3G_CallEventRecord* pBackCDR = (CDR3G_CallEventRecord*)pABill;
            uLen = sizeof(CDR3G_CallEventRecordType);
			switch (pBackCDR->choice)
			{
			case 1:
    ...
    else if(sFieldName == "chargingCharacteristics")
	{
		...
		if (m_bIsMustMerge)
		{
			...
			{
			case 1:
				if (pBackCDR->u.sgsnPDPRecord.bit_mask & _3GCDR_SGSNPDPRecord_chargingCharacteristics_present)
				{
			...
			case 2:
				if (pBackCDR->u.ggsnPDPRecord.bit_mask & _3GCDR_GGSNPDPRecord_chargingCharacteristics_present)
				{
			...
			case 3:
				if (pBackCDR->u.sgsnMMRecord.bit_mask & _3GCDR_SGSNMMRecord_chargingCharacteristics_present)
				{
			...
			case 4:
				if (pBackCDR->u.sgsnSMORecord.bit_mask & _3GCDR_SGSNSMORecord_chargingCharacteristics_present)
				{
			...
			case 5:
				if (pBackCDR->u.sgsnSMTRecord.bit_mask & _3GCDR_SGSNSMTRecord_chargingCharacteristics_present)
				{
		...
		else
		{
			...
			{
			case 1:
				if (pBackCDR->u.sgsnPDPRecord.bit_mask & CDR3G_SGSNPDPRecord_chargingCharacteristics_present)
				{
					nTemp = pBackCDR->u.sgsnPDPRecord.chargingCharacteristics.value[0] & 0x0f;
				}
				else
			...
			case 2:
				if (pBackCDR->u.ggsnPDPRecord.bit_mask & CDR3G_GGSNPDPRecord_chargingCharacteristics_present)
				{
					nTemp = pBackCDR->u.ggsnPDPRecord.chargingCharacteristics.value[0] & 0x0f;
				}
				else
			...
			case 3:
				if (pBackCDR->u.sgsnMMRecord.bit_mask & CDR3G_SGSNMMRecord_chargingCharacteristics_present)
				{
					nTemp = pBackCDR->u.sgsnMMRecord.chargingCharacteristics.value[0] & 0x0f;
				}
				else
			...
			case 4:
				if (pBackCDR->u.sgsnSMORecord.bit_mask & CDR3G_SGSNSMORecord_chargingCharacteristics_present)
				{
					nTemp = pBackCDR->u.sgsnSMORecord.chargingCharacteristics.value[0] & 0x0f;
				}
				else
			...
			case 5:
				if (pBackCDR->u.sgsnSMTRecord.bit_mask & CDR3G_SGSNSMTRecord_chargingCharacteristics_present)
				{
					nTemp = pBackCDR->u.sgsnSMTRecord.chargingCharacteristics.value[0] & 0x0f;
				}
				else
