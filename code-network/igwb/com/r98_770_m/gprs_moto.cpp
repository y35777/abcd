...
BYTE * CR98_770PatchForMoto::g_pMemDestFile;        //解码内存映射文件指针 
int    CR98_770PatchForMoto::g_nMemDestFileLen;          //解码内存映射文件的长度
//end
...
/*******************************************************************
函数名称：BOOL PreProcess_GPRS(char* szTmpFileName, STRING& sDestFileName)
功能描述：预处理操作
输入参数：
...
/*******************************************************************
函数名称：BOOL ProcessNotMerged_GPRS(char* szTmpFileName, char*& szDestFileName, UINT4 nChannelID)
功能描述：处理不合并的情况
输入参数：
...
/*******************************************************************
函数名称：BOOL ProcessMerged_GPRS(char* szTmpFileName, char*& szDestFileName, UINT4 nChannelID)
功能描述：处理合并的情况
输入参数：
   参数1：char*   szTmpFileName,需要转换的话单文件名
   参数2: char*&  szDestFileName,缺省目标文件名
...
/*******************************************************************
函数名称：BOOL Process_GPRS(char* szTmpFileName, char*& szDsetFileName, UINT4 nChannelID)
功能描述：动态库处理输出的接口实现函数
输入参数：
   参数1：char*   szTmpFileName,需要转换的话单文件名
...
/*******************************************************************
函数名称：extern "C" BOOL Terminate_GPRS()
功能描述：动态库清除输出的接口实现函数
输入参数：
输出参数：无
返回内容：成功返回TRUE,失败返回FALSE
