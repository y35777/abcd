												...
												const Org_TimeStamp* pLargeTime)
{
    ...
    int zz1, yy1, mm1, dd1, hh1, MM1, ss1;	
    int zz2, yy2, mm2, dd2, hh2, MM2, ss2;	//zz1,zz2表示时区
    char szZz[3];
    ...
    memcpy( szZz,pSmallTime->value + 7, 2);
    szZz[2] = '\0';
    zz1 = atoi(szZz);
    zz1 = (pSmallTime->value[6] == '+') ? (zz1) : (-zz1);
    ...
    memcpy( szZz,pLargeTime->value + 7, 2);
    szZz[2] = '\0';
    zz2 = atoi(szZz);
    zz2 = (pLargeTime->value[6] == '+') ? (zz2) : (-zz2);
    ...
    nDiff = nDaySpanInSec + (hh2 - hh1 + zz1 - zz2) * 3600 + (MM2 - MM1) * 60 + (ss2 - ss1);
...
BOOL CGPRS_130_OSS_SCDR::PrepareData()
{
	...
	//直接拷贝apnSelectionMode、accessPointNameOI、servedMSISDN这几个话单域的结构
