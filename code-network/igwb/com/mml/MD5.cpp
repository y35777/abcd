...
//#include <common/imap_iostream.h>
...
void MD5::update(FILE *file){
  ...
  while ((len=fread(tbuffer, 1, 1024, file)) > 0)
    update(tbuffer, len);
...
void MD5::update(std::istream& stream){
  ...
  while (stream.good()){
    stream.read((char *)tbuffer, 1024); // note that return value of read is unusable.
    len=stream.gcount();
    update(tbuffer, len);
  }
...
MD5:: MD5(unsigned char *string, uint4 len, uint1 *key, uint4 keylen){

  init(key, keylen);  // must be called be all constructors
  update(string, len);
  finalize ();
}
...
int MD5::raw_digest(unsigned char *pcBuf){
...
void MD5::init(unsigned char *key, unsigned int len){
  unsigned int iaTempBuf[4];
  finalized=0;  // we just started!
  ...
  // Load magic initialization constants.
  memset((unsigned char *)iaTempBuf, 0, 16);
  memcpy((unsigned char *)iaTempBuf, key, len%16);
  state[0] = 0x67452301|iaTempBuf[0];
  state[1] = 0xefcdab89|iaTempBuf[1];
  state[2] = 0x98badcfe|iaTempBuf[2];
  state[3] = 0x10325476|iaTempBuf[3];
}
