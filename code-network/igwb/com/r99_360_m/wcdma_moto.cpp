...
#include "../r99_360/asn1_r99/CDRF_3GCDR.h"
#include "../r99_360/asn1_r99/CDRF_3GCDR_CG.h"
#include "../r99_360/asn1_r99/oss_asn1_3g.h"
#include "asn1_r99/CDRF_3GCDR_M.h"
#include "asn1_r99/CDRF_3GCDR_CG_M.h"
#include "asn1_r99/oss_asn1_r99.h"
#include "wcdma_moto.h"
#include "../include/export_dll.h"


#define ASN1_DECODE_BUF_SIZE (1024 * 1024 * 4)      //用于进行ASN1解码的内存空间的大小
#define BILL_FILE_SIZE       (1024 * 1024 * 1.5)    //用于存放话单文件的内存空间的大小
#define BUFF_FILE_SIZE       (1024 * 1024 * 1)      //用于存放话单Value数据的内存空间的大小

char  CR99_360PatchForMoto::g_szCgID[15];           //CG的地址
char  CR99_360PatchForMoto::g_szGsnID[15];          //Gsn的地址
BOOL  CR99_360PatchForMoto::g_bMustMerged;          //话单是否合并
BOOL  CR99_360PatchForMoto::g_bCMCC;                //Motorola是否用在CMCC

BYTE* CR99_360PatchForMoto::g_pMemFile;             //内存映射文件指针
UINT4 CR99_360PatchForMoto::g_nMemFileLen;          //内存映射文件的长度
UINT4 CR99_360PatchForMoto::g_nMemDataLen;          //内存映射文件有效数据的长度

void* CR99_360PatchForMoto::g_pASN1Global;          //用于进行对合并的话单进行解码的环境变量
char* CR99_360PatchForMoto::g_pASN1RecContent;      //用于进行对合并的话单形成M结构的编码内存
int   CR99_360PatchForMoto::g_nASN1RecContentLen;   //用于进行对合并的话单形成M结构的编码内存长度

void* CR99_360PatchForMoto::g_pASN1Global_M;        //用于进行对不合并的话单进行解码的环境变量

void* CR99_360PatchForMoto::g_pBSRecord;            //M的话单文件结构
void* CR99_360PatchForMoto::g_pBSSummaryRecord;     //M的话单文件结构的表头部分

BYTE* CR99_360PatchForMoto::g_pMemLink;             //内存映射文件指针
UINT4 CR99_360PatchForMoto::g_nMemLinkLen;          //内存映射文件的长度

//add by zkq 2004-07-08
BYTE * CR99_360PatchForMoto::g_pMemDestFile;        //解码内存映射文件指针 
int    CR99_360PatchForMoto::g_nMemDestFileLen;     //解码内存映射文件的长度
//end
...
extern "C" IGWB_Svc_Export BOOL PC_Init(const char* szConfigPath,
                                        ...
                                        UINT4 nChannelCount)
{
    return CR99_360PatchForMoto::Init(szConfigPath, nApID, nChannelCount);
}
extern "C" IGWB_Svc_Export BOOL PC_Process(const char* szFileName,
                                           char*& szDestFileName,
                                           UINT4 nChannelNum)
{
    return CR99_360PatchForMoto::Process(szFileName, szDestFileName, nChannelNum);
}
extern "C" IGWB_Svc_Export BOOL PC_Terminate()
{
    return CR99_360PatchForMoto::Terminate();
}

/*******************************************************************
函数名称：extern "C" BOOL Init_WCDMA()
...
********************************************************************/
BOOL CR99_360PatchForMoto::Init(const char* szConfigPath,
                                BYTE nApID,
                                UINT4 nChannelCount)
{
    g_bMustMerged        = TRUE;
    ...
    g_nMemFileLen          = BILL_FILE_SIZE;
    g_pMemFile             = new BYTE[BILL_FILE_SIZE];
    memset(g_pMemFile, 0, BILL_FILE_SIZE);

    g_nASN1RecContentLen   = ASN1_DECODE_BUF_SIZE;
    g_pASN1RecContent      = new char[g_nASN1RecContentLen];
    memset(g_pASN1RecContent, 0, g_nASN1RecContentLen);

    g_nMemLinkLen          = BUFF_FILE_SIZE;
    g_pMemLink             = new BYTE[BUFF_FILE_SIZE];
    memset(g_pMemLink, 0, BUFF_FILE_SIZE);
    ...
    if (g_bMustMerged)
    {
        ...
        g_pBSRecord          = (void*)new _3GCDR_M_BSRecord;
        g_pBSSummaryRecord   = (void*)new _3GCDR_M_BSXferSummaryRecord;
    }
    else
    {
        ...
        g_pBSRecord          = (void*)new CDR3G_M_BSRecord;
        g_pBSSummaryRecord   = (void*)new CDR3G_M_BSXferSummaryRecord;
    }
...
/*******************************************************************
函数名称：BOOL PreProcess_WCDMA(char* szTmpFileName, STRING& sDestFileName)
功能描述：预处理操作
输入参数：
...
********************************************************************/
BOOL CR99_360PatchForMoto::PreProcess(const char* szTmpFileName,
                                      STRING& sDestFileName)
{
    ...
    else
    {
        ...
        if(g_nMemFileLen - sizeof(_3GCDR_M_BSXferSummaryRecord) < g_nMemDataLen)
        {
            g_nMemFileLen = g_nMemDataLen + sizeof(_3GCDR_M_BSXferSummaryRecord);
            delete [] g_pMemFile;
...
/*******************************************************************
函数名称：BOOL ProcessNotMerged_WCDMA(char* szTmpFileName, char*& szDestFileName, UINT4 nChannelID)
功能描述：处理不合并的情况
输入参数：
...
********************************************************************/
BOOL CR99_360PatchForMoto::ProcessNotMerged(const char* szTmpFileName,
                                            char*& szDestFileName,
                                            UINT4 nChannelID)
{
    //进行预处理
    ...
    UINT4 nSequence  = 0;
    BOOL  bIsFirst   = TRUE;
    UINT4 nOrgLen = sizeof(CDR3G_M_CallEventRecord);
    UINT4 nFinBufUsed = 0;
    ...
    while (nCurOffset < g_nMemDataLen)
    {
        ...
        if (ERR_SUCCESS != nRet)
        {
            if(g_nASN1RecContentLen - nFinBufUsed < 1024 * 1024)
            {
                ...
                nSequence  = 0;
                bIsFirst = TRUE;
                nOrgLen = sizeof(CDR3G_M_CallEventRecord);
                nFinBufUsed = 0;
            }
        ...
        //处理每张话单
        CDR3G_M_CallEventRecord* pTemp = (CDR3G_M_CallEventRecord*)(g_pASN1RecContent + nFinBufUsed);

        memcpy(g_pMemLink + nBufOffset, (char*)pTemp + nOrgLen, nDataLen - nOrgLen);

        switch (pTemp->choice)
        {
        case CDR3G_M_sgsnPDPRecord_chosen:
            {
                CDR3G_M_SGSNPDPRecord* p = &pTemp->u.sgsnPDPRecord;
                //如果是第一张话单
                if (bIsFirst)
                {
                    if(p->bit_mask & CDR3G_M_SGSNPDPRecord_localSequenceNumber_present)
                    {
                ...
                if(p->bit_mask & CDR3G_M_SGSNPDPRecord_diagnostics_present)
                {
                    if(p->diagnostics.choice & CDR3G_M_networkSpecificCause_chosen)
                    {
                    ...
                    if(p->diagnostics.choice & CDR3G_M_manufacturerSpecificCause_chosen)
                    {
                ...
                {
                    p->listOfTrafficVolumes.value = (CDR3G_M_ChangeOfCharCondition*)(g_pMemLink + nBufOffset +
                                                    int((char*)p->listOfTrafficVolumes.value - (char*)pTemp) - nOrgLen);
                }

                if(p->bit_mask & CDR3G_M_SGSNPDPRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
                        p->recordExtensions.value = (CDR3G_M_ManagementExtension*)(g_pMemLink + nBufOffset +
                                                    int((char*)p->recordExtensions.value - (char*)pTemp) - nOrgLen);
                        int i;
                        for(i = 0; i < p->recordExtensions.count; i++)//by zkq 2004-07-13 SWPD06735
                        {
                            CDR3G_M_ManagementExtension *value = &p->recordExtensions.value[i];
        ...
        case CDR3G_M_ggsnPDPRecord_chosen:
            {
                CDR3G_M_GGSNPDPRecord* p = &pTemp->u.ggsnPDPRecord;
                //如果是第一张话单
                if (bIsFirst)
                {
                    if(p->bit_mask & CDR3G_M_GGSNPDPRecord_localSequenceNumber_present)
                    {
                        nLrsnStart = p->localSequenceNumber;
                    }
                    //获得第一张话单类型szCDRType
                ...
                {
                    p->sgsnAddress.value = (CDR3G_M_GSNAddress*)(g_pMemLink + nBufOffset +
                                           int((char*)p->sgsnAddress.value - (char*)pTemp) - nOrgLen);
                }

                if(p->bit_mask & CDR3G_M_GGSNPDPRecord_diagnostics_present)
                {
                    if(p->diagnostics.choice & CDR3G_M_networkSpecificCause_chosen)
                    {
                        if((p->diagnostics.u.networkSpecificCause.identifier.value != NULL)
                ...
                if(p->bit_mask & CDR3G_M_GGSNPDPRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
        ...
        case CDR3G_M_sgsnMMRecord_chosen:
            {
                CDR3G_M_SGSNMMRecord* p = &pTemp->u.sgsnMMRecord;

                //如果是第一张话单
                if (bIsFirst)
                {
                    if(p->bit_mask & CDR3G_M_SGSNMMRecord_localSequenceNumber_present)
                    {
                        nLrsnStart = p->localSequenceNumber;
                    }
                    //获得第一张话单类型szCDRType
                ...
                if(p->bit_mask & CDR3G_M_changeLocation_present)
                {
                    if((p->changeLocation.value != NULL)
                        && (p->changeLocation.count > 0))
                    {
                        p->changeLocation.value = (CDR3G_M_ChangeLocation*)(g_pMemLink + nBufOffset +
                                                  int((char*)p->changeLocation.value - (char*)pTemp) - nOrgLen);
                    }
                }

                if(p->bit_mask & CDR3G_M_SGSNMMRecord_diagnostics_present)
                {
                    if(p->diagnostics.choice & CDR3G_M_networkSpecificCause_chosen)
                    {
                        if((p->diagnostics.u.networkSpecificCause.identifier.value != NULL)
                ...
                if(p->bit_mask & CDR3G_M_SGSNMMRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
        ...
        case CDR3G_M_sgsnSMORecord_chosen:
            {
                CDR3G_M_SGSNSMORecord* p = &pTemp->u.sgsnSMORecord;
                //如果是第一张话单
                if (bIsFirst)
                {
                   if(p->bit_mask & CDR3G_M_SGSNSMORecord_localSequenceNumber_present)
                    {
                        nLrsnStart = p->localSequenceNumber;
                    }
                ...
                if(p->bit_mask & CDR3G_M_SGSNSMORecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
        ...
        case CDR3G_M_sgsnSMTRecord_chosen:
            {
                CDR3G_M_SGSNSMTRecord* p = &pTemp->u.sgsnSMTRecord;
                //如果是第一张话单
                if (bIsFirst)
                {
                    if(p->bit_mask & CDR3G_M_SGSNSMTRecord_localSequenceNumber_present)
                    {
                        nLrsnStart = p->localSequenceNumber;
                    }
                    //获得第一张话单类型szCDRType
                ...
                if(p->bit_mask & CDR3G_M_SGSNSMTRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
    ...
    CDR3G_M_BSXferSummaryRecord* pSummary =
        (CDR3G_M_BSXferSummaryRecord*)g_pBSSummaryRecord;
    ...
    //填充结构表头
    ((CDR3G_M_BSRecord*)g_pBSRecord)->bsXferRecord    = *pSummary;
    ((CDR3G_M_BSRecord*)g_pBSRecord)->listOfCDR.count = nTotalCount;
    ((CDR3G_M_BSRecord*)g_pBSRecord)->listOfCDR.value = (CDR3G_M_CallEventRecord*)g_pASN1RecContent;
...
********************************************************************/
BOOL CR99_360PatchForMoto::ProcessMerged(const char* szTmpFileName,
                                         char*& szDestFileName,
                                         UINT4 nChannelID)
{
    //进行预处理
    ...
    UINT4 nSequence  = 0;
    BOOL  bIsFirst   = TRUE;
    UINT4 nFinLen = sizeof(_3GCDR_M_CallEventRecord);
    UINT4 nFinBufUsed = 0;

	 //added by zkq  2004-06-10
	//判断是否为"exclude,excluded"
	int nStart = -1;
	int nEnd   = -1;
	int nSeq   = -1;
	//获得目标目录的文件夹名判断计费特性	
    ...
    while (nCurOffset < g_nMemDataLen)
    {
        ...
        if (ERR_SUCCESS != nRet)
        {
            if(g_nASN1RecContentLen - nFinBufUsed < 1024 * 1024)
            {
                ...
                nSequence  = 0;
                bIsFirst = TRUE;
                nFinLen = sizeof(_3GCDR_M_CallEventRecord);
                nFinBufUsed = 0;
            }
        ...
        _3GCDR_M_CallEventRecord* pTemp = (_3GCDR_M_CallEventRecord*)(g_pASN1RecContent + nFinBufUsed);

        memcpy(g_pMemLink + nBufOffset, (char*)pTemp + nFinLen, nDataLen - nFinLen);

        switch (pTemp->choice)
        {
        case _3GCDR_M_sgsnPDPRecord_chosen:
            {
                //如果是第一张话单
                if (bIsFirst)
                {
                ...
                _3GCDR_M_SGSNPDPRecord* p = &pTemp->u.sgsnPDPRecord;

                if(p->bit_mask & _3GCDR_M_SGSNPDPRecord_diagnostics_present)
                {
                ...
                if((p->listOfTrafficVolumes.value != NULL)
                    && (p->listOfTrafficVolumes.count > 0))
                {
                    p->listOfTrafficVolumes.value = (_3GCDR_M_ChangeOfCharCondition*)(g_pMemLink + nBufOffset +
                                                    int((char*)p->listOfTrafficVolumes.value - (char*)pTemp) - nFinLen);
                }

                if(p->bit_mask & _3GCDR_M_SGSNPDPRecord_recSequenceNumList_present)
                {
                ...
                if(p->bit_mask & _3GCDR_M_SGSNPDPRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
        ...
        case _3GCDR_M_ggsnPDPRecord_chosen:
            {
                //如果是第一张话单
                if (bIsFirst)
                {
                ...
                _3GCDR_M_GGSNPDPRecord* p = &pTemp->u.ggsnPDPRecord;

                if((p->sgsnAddress.value != NULL)
                    && (p->sgsnAddress.count > 0))
                {
                    p->sgsnAddress.value = (_3GCDR_M_GSNAddress*)(g_pMemLink + nBufOffset +
                                           int((char*)p->sgsnAddress.value - (char*)pTemp) - nFinLen);
                }

                if(p->bit_mask & _3GCDR_M_GGSNPDPRecord_diagnostics_present)
                {
                    if(p->diagnostics.choice & _3GCDR_M_networkSpecificCause_chosen)
                    {
                        if((p->diagnostics.u.networkSpecificCause.identifier.value != NULL)
                ...
                if(p->bit_mask & _3GCDR_M_GGSNPDPRecord_recSequenceNumList_present)
                {
                    if(p->recSequenceNumList.bit_mask & _3GCDR_M_recSequenceNumber_present)
                    {
                        if((p->recSequenceNumList.recSequenceNumber.value != NULL)
                ...
                if(p->bit_mask & _3GCDR_M_GGSNPDPRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
        ...
        case _3GCDR_M_sgsnMMRecord_chosen:
            {
                ...
                if(p->bit_mask &  _3GCDR_M_SGSNMMRecord_localSequenceNumber_present)
                {
					nSeq = 1; //aded by zkq 2004-06-10
                    nSequence = p->localSequenceNumber;
                }
        ...
        */
        case _3GCDR_M_sgsnSMORecord_chosen:
            {
                _3GCDR_M_SGSNSMORecord* p = &pTemp->u.sgsnSMORecord;
                //如果是第一张话单
                if (bIsFirst)
                {
                    //获得第一张话单的序列号nLrsnStart
                    if(p->bit_mask & _3GCDR_M_SGSNSMORecord_localSequenceNumber_present)
                    {
                ...
                if(p->bit_mask & _3GCDR_M_SGSNSMORecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
        ...
        case _3GCDR_M_sgsnSMTRecord_chosen:
            {
                _3GCDR_M_SGSNSMTRecord* p = &pTemp->u.sgsnSMTRecord;
                //如果是第一张话单
                if (bIsFirst)
                {
                    //获得第一张话单的序列号nLrsnStart
                    if(p->bit_mask & _3GCDR_M_SGSNSMTRecord_localSequenceNumber_present)
                    {
						nStart = 1; //aded by zkq 2004-06-10
                        nLrsnStart = p->localSequenceNumber;
                    }
                ...
                if(p->bit_mask & _3GCDR_M_SGSNSMTRecord_recordExtensions_present)
                {
                    if((p->recordExtensions.value != NULL)
                        && (p->recordExtensions.count > 0))
                    {
    ...
    _3GCDR_M_BSXferSummaryRecord* pSummary = (_3GCDR_M_BSXferSummaryRecord*)g_pBSSummaryRecord;
    ...
    //填充结构表头
    ((_3GCDR_M_BSRecord*)g_pBSRecord)->bsXferRecord    = *pSummary;
    ((_3GCDR_M_BSRecord*)g_pBSRecord)->listOfCDR.count = nTotalCount;
    ((_3GCDR_M_BSRecord*)g_pBSRecord)->listOfCDR.value = (_3GCDR_M_CallEventRecord*)g_pASN1RecContent;
...
/*******************************************************************
函数名称：BOOL Process(cosnt char* szTmpFileName, char*& szDsetFileName, UINT4 nChannelID)
功能描述：动态库处理输出的接口实现函数
输入参数：
   参数1：char*   szTmpFileName,需要转换的话单文件名
...
********************************************************************/
BOOL CR99_360PatchForMoto::Process(const char* szTmpFileName,
                                   char*& szDsetFileName,
                                   UINT4 nChannelID)
{
    if (g_bMustMerged)
...
********************************************************************/
BOOL CR99_360PatchForMoto::Terminate()
{
    if (g_pMemFile != NULL)
    {
        delete[] g_pMemFile;
