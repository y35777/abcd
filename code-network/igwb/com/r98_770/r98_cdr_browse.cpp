...
#define GPRSCAUSEFORRECCLOSINGENUNINFO_LENGTH   7
...
#define R98_770_RECORDE_TYPE_ENUNINFO_LENGTH 5
static ENUMITEMINFO R98_770_Recorde_Type_EnumInfo[] =
{
    18,	"S-CDR",
    19,	"G-CDR",
    20,	"M-CDR",
...
#define SCDRBUFFERLENGTH _BROWSETEMPBUFFERLENGTH
    ...
    //recordType
    ENUMINFO EnumInfo;
    EnumInfo.nCount = R98_770_RECORDE_TYPE_ENUNINFO_LENGTH;
    EnumInfo.pInfoTable = R98_770_Recorde_Type_EnumInfo;
...
#define GCDRBUFFERLENGTH _BROWSETEMPBUFFERLENGTH
    ...
    if (pBill->bit_mask & Org_GGSNPDPRecord_recordExtensions_present)
    {
		...
		case CBB_OPERATOR_ID_CMCC:
		    nLen += CMCC_R98_GCDRRecordExtensionsToStr(pBill->recordExtensions, 
                        &pTempBuffer[nLen], GCDRBUFFERLENGTH - nLen);
		    break;
			    
		default:
    		nLen += R98_GCDRRecordExtensionsToStr(pBill->recordExtensions, 
                       &pTempBuffer[nLen], GCDRBUFFERLENGTH - nLen);
            break;			    
        }
    }
	...
	//added by ggf,2003-11-19
    //sgsnPLMNIdentifier
	if (pBill->bit_mask & Org_sgsnPLMNIdentifier_present)
    {
...
int GetTextValueOfGPRSLARGEGGSNPDP(char *pValue, char *pBuffer, int nLength)
{
	...
	//added by ggf,2003-11-19
    //sgsnPLMNIdentifier
	if (pBill->bit_mask & Fin_sgsnPLMNIdentifier_present)
    {
        //by ldf 2004-03-26 对应问题单SWPD03933
        nLen += CvtBCDOctecToStr2((LPOCTECSTRING)&pBill->sgsnPLMNIdentifier, &pTempBuffer[nLen], nLength - nLen);
        //end
...
int GetTextValueOfGPRSLARGESGSNMM(char *pValue, char *pBuffer, int nLength)
{
    ...
    {
        nLen += Gprs_RecordExtensionsToStr((struct Org_ManagementExtensions_ *)&pBill->recordExtensions, &pTempBuffer[nLen],
                                        MCDRBUFFERLENGTH - nLen);
    }
    else
    {
...
int Gprs_RecordExtensionToStr(Org_ManagementExtension *pData, char *pBuffer, int nLength)
{
    ...
    nLen += HexOctecToStr((LPOCTECSTRING)&pData->identifier, &pTempBuffer[nLen],
                            nLength - nLen, "0x");
    pTempBuffer[nLen++] = ' ';
    ...
    nLen += HexOctecToStr((LPOCTECSTRING)&pData->information, &pTempBuffer[nLen],
                            nLength - nLen, "0x");
...
/******************************************************
// Function name: R98_GCDRRecordExtensionsToStr
// Author:        
// Date:          2003.12.30
// Description:   转换Org_ManagementExtensions_类型的数据为字符串
...
#define SCDRBUFFERLENGTH _BROWSETEMPBUFFERLENGTH
    ...
    if(NULL != pValues)
    {
        ...
        for(int i = 0; i < pValues->size(); i++)
        {
            ...
            {
                _GPRSPDPAddrToStr(&pBill->servedPDPAddress,
                    pTempBuffer,
                    nLength);
                value.sFieldValue = pTempBuffer;
                nLen++;
                                      ...
                                      QUERY_COND_FIELDS_VALUE* pValues)
{
    ...
    if(NULL != pValues)
    {
        ...
        for(int i = 0; i < pValues->size(); i++)
        {
            ...
            {
                _GPRSPDPAddrToStr((Org_PDPAddress*)&pBill->servedPDPAddress,
                    pTempBuffer,
                    nLength);
                value.sFieldValue = pTempBuffer;
                nLen++;
